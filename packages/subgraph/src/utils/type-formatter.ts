...
export function formatAddressArrayToBytesArray(addressArray: Address[]): Bytes[] {
  const bytesArray: Array<Bytes> = [];
...
export function pushToBigIntArray(array: BigInt[] | null, adder: BigInt): BigInt[] {
  if (array == null) {
    return [adder];
  } else {
