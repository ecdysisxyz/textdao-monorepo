...
import { genHeaderContentsId, genHeaderId, genProposalId } from "../../src/utils/entity-id-provider";
import { createMockHeaderCreatedEvent } from "../utils/mock-events";

function assertProposalFieldFilledWithMetadata1(headerContentsEntityId: string): void {
  assert.fieldEquals("HeaderContents", headerContentsEntityId, "title", "Sample Header Title1");
  assert.fieldEquals("HeaderContents", headerContentsEntityId, "body", "wrrrrrrrrrrryyyy");
}
function assertProposalFieldFilledWithMetadata2(headerContentsEntityId: string): void {
  assert.fieldEquals("HeaderContents", headerContentsEntityId, "title", "Sample Header Title2");
  assert.fieldEquals("HeaderContents", headerContentsEntityId, "body", "foooooooooooo");
}

describe("HeaderCreated Event Handler", () => {
  const metadataCid1 = "QmTest1";
  const metadataFilePath1 = "tests/utils/ipfs-file-data/sample-proposal-header-metadata1.json";
  const metadataCid2 = "QmTest2";
  const metadataFilePath2 = "tests/utils/ipfs-file-data/sample-proposal-header-metadata2.json";
  ...
  test("Should create multiple Header entities for different proposals", () => {
    const pids: BigInt[] = [BigInt.fromI32(100), BigInt.fromI32(101), BigInt.fromI32(100)];
    const headerIds: BigInt[] = [BigInt.fromI32(1), BigInt.fromI32(1), BigInt.fromI32(2)];
    const metadataCids: string[] = [metadataCid1, metadataCid2, metadataCid1];

    for (let i = 0; i < pids.length; i++) {
      // logDataSources("HeaderContents");

      handleHeaderCreated(createMockHeaderCreatedEvent(pids[i], headerIds[i], metadataCids[i]));
  ...
  test(
    ...
    () => {
      ...
      handleHeaderCreated(createMockHeaderCreatedEvent(pid, headerId, metadataCid1));
      handleHeaderCreated(createMockHeaderCreatedEvent(pid, headerId, metadataCid2));
    },
    true,
  );
  ...
  test("Should handle Headers with empty metadataCid", () => {
    ...
    handleHeaderCreated(createMockHeaderCreatedEvent(pid, headerId, emptyMetadataCid));
