...
describe("Proposed Event Handler", () => {
  ...
  test(
    ...
    () => {
      const pid = BigInt.fromI32(100);
      const proposer = Address.fromString("0x1234567890123456789012345678901234567890");
      const createdAt = BigInt.fromI32(1625097600);
      const expirationTime = BigInt.fromI32(1625184000);
      const snapInterval = BigInt.fromI32(72000);
  ...
  test("Should handle multiple RepresentativesAssigned and Proposed events in correct order", () => {
    const pid = BigInt.fromI32(100);
    const proposalEntityId = genProposalId(pid);
    const initialReps = [Address.fromString("0x1111111111111111111111111111111111111111")];
    const proposer = Address.fromString("0x2222222222222222222222222222222222222222");
    const createdAt = BigInt.fromI32(1625097600);
    const expirationTime = BigInt.fromI32(1625184000);
    const snapInterval = BigInt.fromI32(72000);
    const finalReps = [
    ...
    handleRepresentativesAssigned(createMockRepresentativesAssignedEvent(pid, initialReps));
    handleProposed(createMockProposedEvent(pid, proposer, createdAt, expirationTime, snapInterval));
    handleRepresentativesAssigned(createMockRepresentativesAssignedEvent(pid, finalReps));
    ...
    assert.fieldEquals("Proposal", proposalEntityId, "createdAt", createdAt.toString());
    assert.fieldEquals("Proposal", proposalEntityId, "expirationTime", expirationTime.toString());
    assert.fieldEquals("Proposal", proposalEntityId, "reps", formatAddressArray(finalReps));
  });
  ...
  test("Should handle proposal with zero address proposer", () => {
    ...
    handleProposed(createMockProposedEvent(pid, zeroAddress, createdAt, expirationTime, snapInterval));

    const proposalEntityId = genProposalId(pid);
    assert.fieldEquals("Proposal", proposalEntityId, "proposer", zeroAddress.toHexString());
  });
