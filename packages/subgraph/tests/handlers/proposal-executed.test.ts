...
import { createMockActionEntity, createMockCommandEntity, createMockProposalEntity } from "../utils/mock-entities";
...
describe("ProposalExecuted Event Handler", () => {
  ...
  test("Should mark proposal as fully executed and update action statuses", () => {
    ...
    assert.fieldEquals("Action", genActionId(pid, approvedCommandId, 0), "status", "Executed");
    assert.fieldEquals("Action", genActionId(pid, approvedCommandId, 1), "status", "Executed");
  });
  ...
  test("Should not affect other proposals or commands", () => {
    ...
    handleProposalExecuted(createMockProposalExecutedEvent(pid1, approvedCommandId));

    // Assert pid1 proposal is fully executed
    assert.fieldEquals("Proposal", proposalId1, "fullyExecuted", "true");
    assert.fieldEquals("Action", genActionId(pid1, approvedCommandId, 0), "status", "Executed");

    // Assert pid2 proposal is not affected
    assert.fieldEquals("Proposal", proposalId2, "fullyExecuted", "false");
    assert.fieldEquals("Action", genActionId(pid2, approvedCommandId, 0), "status", "Proposed");
  });
