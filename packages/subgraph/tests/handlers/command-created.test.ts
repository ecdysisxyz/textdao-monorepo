...
describe("CommandCreated Event Handler", () => {
  ...
  test("Should create and store a single Command entity with its Actions and createdAt timestamp", () => {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      assert.fieldEquals("Action", actionEntityId, "command", commandEntityId);
      assert.fieldEquals("Action", actionEntityId, "func", actions[i].funcSig);
      assert.fieldEquals("Action", actionEntityId, "abiParams", actions[i].abiParams.toHexString());
  ...
  test("Should create multiple Command entities for different proposals", () => {
    ...
    for (let i = 0; i < pids.length; i++) {
      handleCommandCreated(createMockCommandCreatedEvent(pids[i], commandIds[i], actions[i]));
      ...
      for (let j = 0; j < actions[i].length; j++) {
        ...
        assert.fieldEquals("Action", actionEntityId, "id", actionEntityId);
        assert.fieldEquals("Action", actionEntityId, "command", commandEntityId);
        assert.fieldEquals("Action", actionEntityId, "func", actions[i][j].funcSig);
        assert.fieldEquals("Action", actionEntityId, "abiParams", actions[i][j].abiParams.toHexString());
        assert.fieldEquals("Action", actionEntityId, "status", "Proposed");
      }
    }
  });
  ...
  test("Should create a Proposal entity if it doesn't exist", () => {
    const pid = BigInt.fromI32(100);
    const commandId = BigInt.fromI32(1);
    const actions: Action[] = [new Action("function1()", Bytes.fromHexString("0x1234"))];
  ...
  test(
    ...
    () => {
      const pid = BigInt.fromI32(100);
      const commandId = BigInt.fromI32(1);
      const initialActions: Action[] = [new Action("function1()", Bytes.fromHexString("0x1234"))];
      const updatedActions: Action[] = [new Action("function2()", Bytes.fromHexString("0x5678"))];

      handleCommandCreated(createMockCommandCreatedEvent(pid, commandId, initialActions));
      handleCommandCreated(createMockCommandCreatedEvent(pid, commandId, updatedActions));
    },
    true,
  );
  ...
  test("Should handle Commands with empty actions", () => {
    ...
    handleCommandCreated(createMockCommandCreatedEvent(pid, commandId, emptyActions));
  ...
  test("Should handle multiple Commands for the same Proposal", () => {
    const pid = BigInt.fromI32(100);
    const commandIds: BigInt[] = [BigInt.fromI32(1), BigInt.fromI32(2), BigInt.fromI32(3)];
    const actions: Action[][] = [
      [new Action("function1()", Bytes.fromHexString("0x1234"))],
      [new Action("function2()", Bytes.fromHexString("0x5678"))],
      [new Action("function3()", Bytes.fromHexString("0x9abc"))],
    ...
    for (let i = 0; i < commandIds.length; i++) {
      handleCommandCreated(createMockCommandCreatedEvent(pid, commandIds[i], actions[i]));
      ...
      assert.fieldEquals("Action", actionEntityId, "command", commandEntityId);
      assert.fieldEquals("Action", actionEntityId, "func", actions[i][0].funcSig);
      assert.fieldEquals("Action", actionEntityId, "abiParams", actions[i][0].abiParams.toHexString());
    }
  });
});
