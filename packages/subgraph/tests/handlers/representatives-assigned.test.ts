...
describe("RepresentativesAssigned Event Handler", () => {
  ...
  test("Should update existing Proposal entity with new representatives", () => {
    ...
    handleRepresentativesAssigned(createMockRepresentativesAssignedEvent(pid, newReps));

    assert.entityCount("Proposal", 1);
    assert.fieldEquals("Proposal", proposalEntityId, "reps", formatAddressArray(newReps));
  });
  ...
  test("Should handle empty representatives array", () => {
    ...
    handleRepresentativesAssigned(createMockRepresentativesAssignedEvent(pid, emptyReps));

    assert.entityCount("Proposal", 1);
    assert.fieldEquals("Proposal", proposalEntityId, "reps", formatAddressArray(emptyReps));
  });
});
