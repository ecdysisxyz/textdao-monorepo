...
import { assert, beforeEach, clearStore, dataSourceMock, describe, readFile, test } from "matchstick-as/assembly/index";
import {
  handleMemberAdded,
...
import { genMemberId, genMemberInfoId } from "../../src/utils/entity-id-provider";
import {
...
describe("Member Event Handlers", () => {
  ...
  const metadataFilePath1 = "tests/utils/ipfs-file-data/sample-member-metadata1.json";
  const metadataCid2 = "QmTest2";
  const metadataFilePath2 = "tests/utils/ipfs-file-data/sample-member-metadata2.json";
  ...
  test("Should create new Member entity on MemberAddedByProposal event", () => {
    ...
    const memberEntityId = genMemberId(memberId);
    const memberInfoId = genMemberInfoId(metadataCid1);
    const addr = Address.fromString("0x2345678901234567890123456789012345678901");

    dataSourceMock.setAddress(metadataCid1);
    handleMemberAddedByProposal(createMockMemberAddedByProposalEvent(memberId, addr, metadataCid1));
    handleMemberInfo(readFile(metadataFilePath1));
  ...
  test("Should update existing Member entity on MemberUpdated event", () => {
    ...
    const addr1 = Address.fromString("0x1234567890123456789012345678901234567890");
    const addr2 = Address.fromString("0xaaaa567890123456789012345678901234567890");

    dataSourceMock.setAddress(metadataCid1);
    handleMemberAdded(createMockMemberAddedEvent(memberId, addr1, metadataCid1));
    handleMemberInfo(readFile(metadataFilePath1));
    ...
    handleMemberUpdated(createMockMemberUpdatedEvent(memberId, addr2, metadataCid2));
    ...
    assert.fieldEquals("MemberInfo", memberInfoId2, "bio", "I'm an updated user.");
  });
  ...
  test("Should update existing Member entity on MemberUpdatedByProposal event", () => {
    ...
    dataSourceMock.setAddress(metadataCid2);
    handleMemberUpdatedByProposal(createMockMemberUpdatedByProposalEvent(memberId, addr2, metadataCid2));
    handleMemberInfo(readFile(metadataFilePath2));
  ...
  test("Should handle multiple Member entities", () => {
    ...
    const memberInfoId2 = genMemberInfoId(metadataCid2);
    const addr1 = Address.fromString("0x1234567890123456789012345678901234567890");
    const addr2 = Address.fromString("0x2345678901234567890123456789012345678901");

    dataSourceMock.setAddress(metadataCid1);
    handleMemberAdded(createMockMemberAddedEvent(memberId1, addr1, metadataCid1));
    handleMemberInfo(readFile(metadataFilePath1));
    ...
    dataSourceMock.setAddress(metadataCid2);
    handleMemberAddedByProposal(createMockMemberAddedByProposalEvent(memberId2, addr2, metadataCid2));
    handleMemberInfo(readFile(metadataFilePath2));
  ...
  test("Should handle empty metadataCid with null metadata fields", () => {
    ...
    handleMemberAdded(createMockMemberAddedEvent(memberId, addr, emptyMetadataCid));
  ...
  test("Should mark Member as inactive on MemberRemovedByProposal event", () => {
    ...
    handleMemberAdded(createMockMemberAddedEvent(memberId, addr, metadataCid1));
    handleMemberRemovedByProposal(createMockMemberRemovedByProposalEvent(memberId));
