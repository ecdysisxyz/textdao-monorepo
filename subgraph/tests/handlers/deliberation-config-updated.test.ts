...
import { handleDeliberationConfigUpdated } from "../../src/event-handlers/deliberation-config-updated";
import { createMockDeliberationConfigUpdatedEvent } from "../utils/mock-events";
import { DeliberationConfig } from "../../generated/schema";
import { genDeliberationConfigId } from "../../src/utils/entity-id-provider";

describe("DeliberationConfigUpdated Event Handler", () => {
    beforeEach(() => {
        clearStore();
    });
    ...
    test("Should update existing DeliberationConfig entity", () => {
        ...
        assert.fieldEquals(
            ...
            genDeliberationConfigId(),
            "expiryDuration",
            newExpiryDuration.toString()
        );
        assert.fieldEquals(
            "DeliberationConfig",
            genDeliberationConfigId(),
            "snapInterval",
            newSnapInterval.toString()
        );
        assert.fieldEquals(
            "DeliberationConfig",
            genDeliberationConfigId(),
